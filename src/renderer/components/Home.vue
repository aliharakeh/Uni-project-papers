<template>
  <v-container fluid>
    ...
    <!-- choose template -->
    <v-layout row wrap justify-space-around v-if="doc">
      <v-flex xs6 v-for="template in templates" :key="template.title">
        <v-hover>
          <v-card
              class="ma-3"
              :to="template.link + '/' + doc._id"
              raised
              slot-scope="{ hover }"
...
export default {
  ...
  created () {
    ...
    if (this.$store.getters.selectedDoc) {
      this.doc = this.$store.getters.selectedDoc
    }
  },
  methods: {
    comboBoxText: item => item.name + ' - ' + item.number,
    saveSelectedDoc: function (doc) {
      this.$store.commit('saveSelectedDoc', doc)
    }
  }
}
</script>
